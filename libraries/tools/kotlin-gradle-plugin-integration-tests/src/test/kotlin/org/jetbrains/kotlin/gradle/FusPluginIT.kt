...
class FusPluginIT : KGPBaseTest() {
    ...
    @DisplayName("test with different classloaders")
    @GradleTest
    fun testDifferentClassloaders(gradleVersion: GradleVersion) {
            ...
            listOf("subProjectA", "subProjectB").forEach { subProject ->
                subProject(subProject).buildGradleKts.modify {
                    """
                        ...
                        project.addGradleConfigurationPhaseMetric { listOf(Metric("metric","value", UniqueId.DEFAULT)) }
                    """.trimIndent()
                }
            }
    ...
    private fun TestProject.validateFusReportFileContains(
        ...
        if (gradleVersion < GradleVersion.version("8.2")) {
            val fusReports = Files.list(projectPath.resolve("$reportRelativePath/kotlin-profile")).toList()
            assertEquals(2, fusReports.size, "Two fus reports should be created: for Kotlin and for Fus plugin")
