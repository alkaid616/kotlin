...
// TODO CallExplanation.of(...) function which returns Pair<T, Explanation>?
...
public fun Explanation.toDefaultMessage(
    ...
    appendExplanations(expressions, render)
}
...
private fun StringBuilder.appendExplanations(
    ...
    val explanations = expressions
        .filterIsInstance<ExplainedExpression>()
        .map { it.explanation }
        .toMutableList()

    var index = 0
    while (index < explanations.size) {
        appendLine()
        appendLine(if (index == 0) "Where:" else "And:")

        val diagram = explanations[index++]
        val expressions = diagram.expressions
        appendDiagram(diagram.source, expressions, render)
        expressions
            .filterIsInstance<ExplainedExpression>()
            .map { it.explanation }
            .let(explanations::addAll)
    }
}
