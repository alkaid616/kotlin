...
abstract class BuilderGeneratorBase(session: FirSession) : FirDeclarationGenerationExtension(session) {
    companion object {
    ...
    private fun createFunctions(classSymbol: FirClassSymbol<*>): Map<Name, List<FirJavaMethod>>? {
        ...
        val builderClassName = builder.className.replace("*", classId.shortClassName.asString())
    ...
    private fun createBuilderClass(classSymbol: FirClassSymbol<*>): FirJavaClass? {
        ...
        val builder = lombokService.getBuilder(classSymbol) ?: return null
        val builderName = Name.identifier(builder.className.replace("*", classSymbol.name.asString()))
        val visibility = builder.visibility.toVisibility()
    ...
    private fun Name.toMethodName(builder: BuilderBase): Name {
