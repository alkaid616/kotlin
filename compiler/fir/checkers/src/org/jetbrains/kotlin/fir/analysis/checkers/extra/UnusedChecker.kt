...
object UnusedChecker : AbstractFirPropertyInitializationChecker(MppCheckerKind.Common) {
    override fun analyze(data: VariableInitializationInfoData, reporter: DiagnosticReporter, context: CheckerContext) {
        ...
        data.graph.traverse(AddAllWrites(data.graph, ownData))
    ...
    private class AddAllWrites(root: ControlFlowGraph, private val data: Data) : ControlFlowGraphVisitorVoidWithContext(root) {
        override fun visitNode(node: CFGNode<*>) {}

        override fun visitVariableDeclarationNode(node: VariableDeclarationNode) {
            if (node.fir.initializer != null) {
                data.unreadWrites[node.fir] = currentNonInPlaceGraph()
            }
            data.variableScopes[node.fir.symbol] = currentNonInPlaceGraph()
