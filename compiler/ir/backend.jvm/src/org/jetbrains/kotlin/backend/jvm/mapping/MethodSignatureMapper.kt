...
class MethodSignatureMapper(private val context: JvmBackendContext, private val typeMapper: IrTypeMapper) {
    ...
    private fun mapSignature(
        ...
        for (parameter in function.allParameters) {
            ...
            writeParameter(
                sw,
                parameter.kind == IrParameterKind.RegularParameter && parameter.isSkippedInGenericSignature,
                type,
                function,
                parameter.kind == IrParameterKind.RegularParameter && materialized
            )
        }
