...
class RootMfvcNode internal constructor(
    ...
    init {
        ...
        oldPrimaryConstructor?.let { requireSameSizes(it.allParameters.size, subnodes.size) }
        requireSameSizes(
            leavesCount,
            newPrimaryConstructor?.allParameters?.size,
            primaryConstructorImpl?.allParameters?.size,
            boxMethod.allParameters.size,
        )
        specializedEqualsMethod.allParameters.filterNot { it.kind == IrParameterKind.DispatchReceiver }.let { regularParameters ->
        ...
        for (function in listOfNotNull(oldPrimaryConstructor, newPrimaryConstructor, primaryConstructorImpl, boxMethod, specializedEqualsMethod)) {
            require(function.allParameters.none { it.kind == IrParameterKind.ExtensionReceiver }) { "Extension receiver is not expected for ${function.render()}" }
            require(function.allParameters.none { it.kind == IrParameterKind.ContextParameter }) { "Context receivers are not expected for ${function.render()}" }
        }
    }
